...
async function main() {
  ...
  // 1. Check that logo.png exists (electron-builder will convert it to ICO automatically)
  const logoPath = path.join(ROOT, 'logo.png');
  if (!fs.existsSync(logoPath)) {
    logError('logo.png не найден в корне проекта!');
    process.exit(1);
  }

  // Remove old build/icon.ico if it exists (causes NSIS errors)
  const oldIco = path.join(ROOT, 'build', 'icon.ico');
  if (fs.existsSync(oldIco)) {
    fs.rmSync(oldIco);
    log('Удалён старый build/icon.ico');
  }
