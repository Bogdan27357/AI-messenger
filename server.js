...
app.get('/api/search', async (req, res) => {
  try {
    ...
    const data = await jamendoFetch(`/tracks/?client_id=${JAMENDO_CLIENT_ID}&format=json&limit=200&offset=${offset}&search=${encodeURIComponent(query)}&include=musicinfo&audioformat=mp32`);
    res.json({ results: normalizeTracks(data.results), hasMore: (data.results || []).length === 200 });
  } catch (err) {
...
app.get('/api/tracks/popular', async (req, res) => {
  try {
    const offset = parseInt(req.query.offset) || 0;
    const data = await jamendoFetch(`/tracks/?client_id=${JAMENDO_CLIENT_ID}&format=json&limit=200&offset=${offset}&order=popularity_total&include=musicinfo&audioformat=mp32`);
    res.json({ results: normalizeTracks(data.results), hasMore: (data.results || []).length === 200 });
  } catch (err) {
...
app.get('/api/tracks/bygenre', async (req, res) => {
  try {
    ...
    const offset = parseInt(req.query.offset) || 0;
    const data = await jamendoFetch(`/tracks/?client_id=${JAMENDO_CLIENT_ID}&format=json&limit=200&offset=${offset}&tags=${encodeURIComponent(genre)}&include=musicinfo&audioformat=mp32&order=popularity_total`);
    res.json({ results: normalizeTracks(data.results), hasMore: (data.results || []).length === 200 });
  } catch (err) {
